...
export default function DashboardStatusTable() {
  const [row1, setRows1] = useState(0); //useState for store rows of table
  const [row2, setRows2] = useState(0); //useState for store rows of table
  ...
  useEffect(() => {
    ...
    async function statusData() {
      ...
      if (status) {
        let ob1 = status.filter((d) => parseInt(d.standard.substring(4,5)) <= 5);
        let ob2 = status.filter((d) => parseInt(d.standard.substring(4,5)) > 5);
        setRows1(ob1);
        setRows2(ob2);
      }
